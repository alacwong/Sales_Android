...
public class DatabaseInserter {
  ...
  /**
   * Port new user without double hashing password.
   ...
   * @throws DatabaseInsertException if there is a failure on the insert
   */
  protected static int portNewUser(String name, int age, String address, String password,
      Connection connection) throws DatabaseInsertException {
    int id = insertUser(name, age, address, connection);
    if (id != -1) {
      portPassword(password, id, connection);
      return id;
    }
    throw new DatabaseInsertException();
  }
  ...
  /**
   * Inserts password for case of reloading database.
   * 
   ...
   * @return
   */
  private static boolean portPassword(String password, int userId, Connection connection) {
