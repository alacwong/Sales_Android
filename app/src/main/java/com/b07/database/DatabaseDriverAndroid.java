...
public class DatabaseDriverAndroid extends SQLiteOpenHelper {
   ...
   */
  protected long portNewUser(String name, int age, String address, String password) {
    long id = insertUser(name, age, address);
    portPassword(password, (int) id);
    return id;
  }
   ...
   */
  private void portPassword(String password, int userId) {
    SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
    ContentValues contentValues = new ContentValues();
